...
{
    timer_idx_t timer_num = (timer_idx_t)hal->dev->hw_timer;
    ...
    timer_ll_set_counter_enable(hal->dev, timer_num, false);
    //timer_ll_enable_auto_reload(hal->dev, hal->timer_id, false);
    //t;imer_ll_set_auto_reload(timg_dev_t *hw, timer_idx_t timer_num, bool auto_reload_en)
    timer_ll_set_auto_reload(hal->dev, timer_num, false);
    ...
    timer_ll_set_alarm_enable(hal->dev, timer_num, false);
...
esp_err_t gptimer_new_timer(const gptimer_config_t *config, gptimer_handle_t *ret_timer)
{
    ...
    timer_ll_set_counter_increase(timer->hal.dev, timer_id, config->direction);
    ...
    //timer_ll_set_alarm_enable(timg_dev_t *hw, timer_idx_t timer_num, bool alarm_en)
    //timer_ll_set_alarm_enable(timer->hal.dev, TIMER_LL_EVENT_ALARM(timer_id), false);
    //timer_ll_intr_disable(timg_dev_t *hw, timer_idx_t timer_num)
    timer_ll_intr_disable(timer->hal.dev, timer_id);

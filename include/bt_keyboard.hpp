...
/*JMH Added the following 2 libraries*/
...
class BTKeyboard
{
  ...
  private:
    ...
    #endif
/*JMH 20230205 - replaced the following two lines xSemaphoreHandle with SemaphoreHandle_t */
...
/*JMH 20230205 - replaced the following line xQueueHandle with QueueHandle_t */
  ...
  public:
    ...
    inline bool wait_for_low_event(KeyInfo & inf, TickType_t duration = portMAX_DELAY) { 
      //printf( "**  wait_for_low_event\n");//for testing/debugging
      // pmsgbx->dispMsg2();//update display now, due to changes related to last key entry 
      // duration = ( TickType_t ) 0x8UL;// ( TickType_t ) 0xffffffffUL
      // ESP_INTR_DISABLE(26);
      // while (!xQueueReceive(event_queue, &inf, duration)){
      //   //printf( "**\n");
      //   pmsgbx->dispMsg2();
      // }
      // ESP_INTR_ENABLE(26);
